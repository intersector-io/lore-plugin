...
1. `node "<hooks dir>/drain-queue.mjs" claim` — prints
   ...
   drain lock or nothing is ready; stop, that is a normal outcome. So does a
   non-zero exit reporting that another process is writing the queue: it
   changed nothing, and several terminals racing here is expected, not an
...
marker with the same algorithm the hook uses (docs/issues/0128):
